...
def main():
    ...
    **Why It Matters:**  
    Traditional CFD simulations can take hours and cost significantly more. Our solution delivers predictions in seconds, saving you both time and money.
    """)
    ...
    if uploaded_file is not None:
        ...
        st.markdown(f"**Predicted Drag ({model_choice}):** {predicted_drag:.8f} (normalized)")
        st.markdown(f"**Inference Time:** {inference_time:.3f} seconds")
        
        # Compare with traditional CFD simulation (example values)
        typical_simulation_time = 7200  # seconds (e.g., 2 hours)
        typical_simulation_cost = 20.0    # dollars per simulation (example)
        time_saved = typical_simulation_time - inference_time
        percent_time_saved = (time_saved / typical_simulation_time) * 100
        st.markdown(f"**Time Saved:** {time_saved:.1f} seconds ({percent_time_saved:.2f}% reduction)")
        st.markdown(f"**Estimated Cost Savings:** Over 99% cost reduction compared to a typical ${typical_simulation_cost:.2f} CFD simulation")
        ...
        # (Optional) Provide a download button for an evaluation plot if available.
